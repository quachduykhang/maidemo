...
function create() {
  form.get(route('blog.create'));
  console.log('hello');
}
function detroy(id){
    if(confirm(' bạn có muốn xóa')){
      form.delete(route('blog.delete',id));
    }
}
</script>
    ...
    <table class="table table-bordered">
      <thead>
        <tr>
          ...
          <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tên Tin tức</th>
          <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mô tả</th>
          <th width="15%" scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Hình ảnh</th>
      ...
      <tbody>
        <tr v-for="(item, index) in blogs" :key="index">
          ...
          <td class="px-6 py-4 whitespace-nowrap"><img :src="'/images/'+ item.images" alt="" srcset=""></td>
